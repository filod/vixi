...
!function($){
	...
	$(function(){
		$('.comment-wrap').on('click','.do-comment',function(e){
			...
			$.post(url,{'content':$txt_comment.val(),
						'wid' : $(this).data('wid'),
...
!function($){ 
	...
	$(function(){
		...
		var poll = function(atonce,callback){
			$.post(url,{'atonce': atonce}).success(function(data){
		...
		}.delay(1000);//DOM ready后，延迟一秒再开始轮询comet服务器
		...
		$noteBtn.on('noticeclear',function(e,data){
			...
			$noteBtn.find('.bubble-dark').text(data[1]).show(); //TODO:闪烁信息数 
			$('.bubble').text(data[1]);
		}); 
		...
		$('.notice-list').on('click','.iknow',function(){
			$self = $(this);
			$.post('/notice/mark',{'noti_uuid':$self.data('uuid')},function(data){
				if ( data.ret === '203'){
